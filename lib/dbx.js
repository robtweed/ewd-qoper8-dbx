/*
 ...
 |                                                                          |
 | Copyright (c) 2019-26 M/Gateway Ltd,                                     |
 | Redhill, Surrey UK.                                                      |
 | All rights reserved.                                                     |
 |                                                                          |
...
module.exports = function(worker, params) {
  ...
  if (options.database.type === 'YottaDB') {
    ...
    if (params.host) options.database.host = params.host;
    if (params.tcp_port) options.database.tcp_port = params.tcp_port;
    if (params.env) options.database.env = params.env;
    if (params.path) options.database.path = params.path;
  }
  ...
  if (options.database.type === 'lmdb') options.database.type = 'LMDB';
  if (options.database.type === 'LMDB') {
    options.database.db_library = params.db_library || "liblmdb.so";
    options.database.env_dir = params.env_dir || "/opt/lmdb";
    options.database.key_type = "m";
  }
  ...
  worker.on('stop', function() {
    ...
    // `this` isn't guaranteed to reference the same object we attached `worker.db` to,
    // so always close via `worker.db`.
    if (worker.db && typeof worker.db.close === 'function') {
      worker.db.close();
    }
